...
디자인과 개발의 영역이 구분되는 건 좋습니다만, 너무 강력하게 영역을 분할할려고하니
오히려 개발할 때 불편을 느낄 수 있을 것 같습니다.    
(물론 MVVM패턴으로 UI구분이되어 유지보수 및 재사용-다중플랫폼에서 사용할 수 있는것은 가장큰 장점입니다.)
...
* **"페이지’의 부분 선언은 다른 기본 클래스를 지정할 수 없습니다."**   
 ...
 LoginPage.g.i.cs를 확인해보면(사진) LoginPage.cs(LoginPage : BasePage)와   
 LoginPage.g.i.cs(LoginPage : Page)가 서로 다른 상속을 받고 있습니다.     
 둘이 같은 상속으로 설정하면 됩니다.    
